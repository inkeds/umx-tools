...
**快速帮助用户生成标准化、文档驱动的 Vibe Coding 文档包，让 AI 开发更稳、更可控、更可落地**。

核心定位：
- 优先用于 CLI 场景（尤其 Claude Code）
- 同时支持传统文档驱动与 Vibe Coding 文件驱动
- 默认从最小文档启动，避免 MVP 过度设计
- 聚焦减少新手 AI 编程常见问题：幻觉、上下文丢失、Bug 循环

## 项目价值：为什么需要这套 Skills

- 用文档包替代“直接让 AI 开写”：先定义边界，再进入实现，降低需求漂移
- 用分层文档承接上下文：避免会话变长后关键信息丢失与重复解释
- 用里程碑和 Spec 约束迭代：降低“修一个坏两个”的 Bug 循环
- 用最小模式先跑通闭环：让新手也能快速进入可交付状态
